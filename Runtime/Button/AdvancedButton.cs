...
namespace Quartzified.UI
{
    ...
    public class AdvancedButton : Selectable, IPointerClickHandler
    {
        ...
        public Sprite enabledSpritePressed;
        [Space]
        public Sprite disabledSprite;
        public Sprite disabledSpritePressed;
        ...
        public UnityEvent OnDisabledClicked;
        
        // Enabled Click or Disabled Click : True / False
        public bool interactState = true;
        ...
        void SetSprite()
        {
            if (IsPressed())
            {
                if (interactState)
                {
                    if (enabledSpritePressed != null)
                        image.sprite = enabledSpritePressed;
                }
                else
                {
                    if (disabledSpritePressed != null)
                        image.sprite = disabledSpritePressed;
                }
            }
            else
            {
                ...
                if (this.IsActive())
                {
                    if (interactState)
                    {
                        if (enabledSound != null)
                            PlayEffectSound(enabledSound);
                    }
                    else
                    {
                        if (disabledSound != null)
                            PlayEffectSound(disabledSound);
                    }
...
#if UNITY_EDITOR
        [MenuItem("GameObject/Convert/Button (UI)/Advanced Button", false, 7)]
        static void ConvertToAdvancedButton(MenuCommand menuCommand)
        {
            ...
            if (button != null)
            {
                ...
                AdvancedButton newButton = holderObj.AddComponent<AdvancedButton>();
        ...
        [MenuItem("GameObject/UI/Advanced Button")]
        static void CreateAdvancedButton(MenuCommand menuCommand)
        {
            // Develope Main Object
            GameObject go = new GameObject("Advanced Button", typeof(RectTransform), typeof(Image), typeof(AdvancedButton));
            GameObjectUtility.SetParentAndAlign(go, menuCommand.context as GameObject);

            go.GetComponent<RectTransform>().sizeDelta = new Vector2(160, 32);
            go.GetComponent<AdvancedButton>().image = go.GetComponent<Image>();
